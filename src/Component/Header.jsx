function Header (props) {

	return (
	  <>
	  <header class="header" >
			  
			  <div class="topbar">
				  <div class="container">
					  <div class="row">
						  <div class="col-lg-6 col-md-5 col-12">
							  
							  <h2>Haider Shahid</h2>
							  
						  </div>
						  <div class="col-lg-6 col-md-7 col-12">
							  
							  <ul class="top-contact">
								  <li><i class="fa fa-phone"></i>011223366</li>
								  <li><i class="fa fa-envelope"></i><a href="mailto:haidershahid3.16@live.com">haidershahid3.16@live.com</a></li>
							  </ul>
							  
						  </div>
					  </div>
				  </div>
			  </div>
			  
			  <div class="header-inner">
				  <div class="container">
					  <div class="inner">
						  <div class="row">
							  <div class="col-lg-3 col-md-3 col-12">
								  
								  <div class="logo">
									  <a href="index.html"><img src="img/Portfolio.png" alt="#"/></a>
								  </div>
								  
								  <div class="mobile-nav"></div>
								  
							  </div>
							  <div class="col-lg-7 col-md-9 col-12">
								  
								  <div class="main-menu">
									  <nav class="navigation">
										  <ul class="nav menu">
											  <li class="active"><a href="#" onClick={()=>props.handlePageChange("About")}>About </a>
												  
											  </li>
											  
											  <li><a href="#" onClick={()=>props.handlePageChange("Portfolio")}>Portfolio </a></li>
											  
											  <li><a href="#" onClick={()=>props.handlePageChange("Resume")}>Resume </a>
												  
											  </li>
											  <li><a href="#" onClick={()=>props.handlePageChange("Contact")}>Contact</a></li>
										  </ul>
									  </nav>
								  </div>
								  
							  </div>
							  
						  </div>
					  </div>
				  </div>
			  </div>
			  
		  </header>
	  </>
	)
  }
  
  export default Header